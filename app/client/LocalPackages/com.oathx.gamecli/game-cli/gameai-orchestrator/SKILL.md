...
策划分析完成后先在当前 Codex 对话展示草案并反复修订。用户明确确认完整版本后，PM 才统一登记美术、程序开发和测试验收需求，各专业一张子任务。单据登记不等于开始制作、编码或测试。程序依赖美术（如有），测试依赖对应交付任务。后续项目管理原样复用 existing_tasks 和 existing_issues，禁止重复创建。
