...
策划产生明确美术、程序开发或测试验收需求后，宿主立即分别登记对应专业单据，不等待整体方案批准；建单不代表授权制作、编码或执行测试。后续完整任务计划必须原样包含输入的 `existing_tasks`，复用 `existing_issues`，禁止重复拆建相同专业范围。程序任务依赖已发现的美术任务，测试任务依赖对应程序与美术任务。没有某专业需求则跳过该专业。恢复优先原样复用 `saved_plan`。
