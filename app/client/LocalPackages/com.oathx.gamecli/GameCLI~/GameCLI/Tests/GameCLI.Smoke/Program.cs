...
namespace GameCLI.Smoke
{
    internal static class Program
    {
        private static async Task Main(string[] args)
        {
            ...
            using JsonDocument endpoint = JsonDocument.Parse(
                await File.ReadAllTextAsync(Path.Combine(project, "Library", "GameCLI", "endpoint.json")));
                ...
                ".", pipeName, PipeDirection.InOut, PipeOptions.Asynchronous))
            {
                ...
                await pipe.WriteAsync(new byte[] { 4, 0 }, timeout.Token);
            }
        ...
        private static async Task CheckRequestAsync(
            string pipeName, string token, string route, string expectedError, CancellationToken cancellation)
        {
            using NamedPipeClientStream pipe = new NamedPipeClientStream(
                ".", pipeName, PipeDirection.InOut, PipeOptions.Asynchronous);
            await pipe.ConnectAsync(cancellation);
            await PipeProtocol.WriteAsync(pipe,
                JsonSerializer.Serialize(new { method = "GET", path = route, token }), cancellation);
            ...
            if (response.RootElement.GetProperty("ok").GetBoolean() ||
                response.RootElement.GetProperty("error").GetString() != expectedError ||
                response.RootElement.TryGetProperty("token", out _))
            {
        ...
        private static async Task RunCliAsync(
            string cli, string project, int expectedExit, params string[] command)
        {
            ...
            if (expectedExit == 0)
            {
                ...
                if (!response.RootElement.GetProperty("ok").GetBoolean() ||
                    response.RootElement.GetProperty("message").GetString() != "pong" ||
