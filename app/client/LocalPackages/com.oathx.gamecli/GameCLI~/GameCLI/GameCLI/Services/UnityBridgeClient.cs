...
namespace GameCLI.Services
{
    internal static class UnityBridgeClient
    {
        ...
        public static async Task<PingResponse> PingAsync(string? projectPath, CancellationToken token)
        {
            ...
            if (endpoint == null || endpoint.ProtocolVersion != 1 ||
                string.IsNullOrWhiteSpace(endpoint.PipeName) || !endpoint.PipeName.StartsWith("gamecli-", StringComparison.Ordinal) ||
                string.IsNullOrWhiteSpace(endpoint.Token) || endpoint.Pid <= 0 ||
                string.IsNullOrWhiteSpace(endpoint.ProjectPath) || !SamePath(endpoint.ProjectPath, root))
            {
            ...
            string request = JsonSerializer.Serialize(new { method = "GET", path = "/ping", token = endpoint.Token });
            ...
            if (response == null || (response.Ok &&
                (response.Message != "pong" || string.IsNullOrWhiteSpace(response.ProjectPath) ||
                 !SamePath(response.ProjectPath, root) || response.Pid != endpoint.Pid ||
                 string.IsNullOrWhiteSpace(response.UnityVersion) || !DateTimeOffset.TryParse(response.RespondedAtUtc, out _))))
            {
