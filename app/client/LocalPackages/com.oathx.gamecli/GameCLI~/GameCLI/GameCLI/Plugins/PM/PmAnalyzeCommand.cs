...
namespace GameCLI.Plugins.PM
{
    internal sealed class PmAnalyzeCommand : ICommand
    {
        ...
        {
            return RunAsync(args.Length == 1 && args[0] is "--help" or "-h" ? args : new[] { "--analyze" }.Concat(args).ToArray(), cancellationToken);
        }
        ...
        private static async Task<int> RunAsync(string[] args, CancellationToken cancellationToken)
        {
            ...
            try
            {
                for (int i = 0; i < args.Length; i++)
                {
                    ...
                    else if (key is "--project" or "--prompt" or "--prompt-file" or "--codex" or "--skills" or "--model" or "--timeout" or "--format" &&
                        i + 1 < args.Length && !args[i + 1].StartsWith("--", StringComparison.Ordinal) && values.TryAdd(key, args[i + 1]))
                    {
                        i++;
                    }
                    else
                ...
                json = format == "json";
                if (!analyze || !values.ContainsKey("--project") || format is not ("human" or "json") ||
                    values.ContainsKey("--prompt") == values.ContainsKey("--prompt-file") ||
                    !int.TryParse(values.GetValueOrDefault("--timeout", "300"), out int seconds) || seconds < 1 || seconds > 3600)
                {
                    throw new ArgumentException(Usage);
                }
                ...
                string prompt = values.TryGetValue("--prompt", out string? inline) ? inline :
                    await File.ReadAllTextAsync(values["--prompt-file"], new UTF8Encoding(false, true));
                ...
                string skills = values.TryGetValue("--skills", out string? skillPath)
                    ? Path.GetFullPath(skillPath) : FindSkills(project);
                foreach (string name in new[] { "gameai-pm", "gameai-common", "gameai-jira" })
                {
                ...
                PmRunResult result = await CodexPmRunner.RunAsync(values.GetValueOrDefault("--codex", "codex"), project,
                    skills, prompt, values.GetValueOrDefault("--model"), traceId, executionId,
                    message => Console.Error.Write(message), cancellation.Token);
                int exitCode = result.Analysis.Status switch { "success" => 0, "blocked" => 3, _ => 2 };
                if (json)
                {
                    Console.WriteLine(JsonSerializer.Serialize(new
                    {
                        ok = exitCode == 0, mode = "read_only_draft", started_at = started,
                        completed_at = DateTimeOffset.UtcNow, result
                ...
                JsonException or OperationCanceledException or Win32Exception or UnauthorizedAccessException or CodexInteractionException)
            {
                ...
                string message = exception is OperationCanceledException
                    ? userCancelled ? "PM analysis cancelled." : "PM analysis timed out."
                    : exception.Message;
                if (json)
                {
                    Console.WriteLine(JsonSerializer.Serialize(new { ok = false, trace_id = traceId, execution_id = executionId, exit_code = code, message }));
                }
        ...
        private static string FindSkills(string project)
        {
            ...
            {
                foreach (string relative in new[] { "game-cli", "LocalPackages/com.oathx.gamecli/game-cli", "app/client/LocalPackages/com.oathx.gamecli/game-cli" })
                {
