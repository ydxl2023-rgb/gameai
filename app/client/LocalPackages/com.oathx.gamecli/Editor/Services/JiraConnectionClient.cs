...
namespace Oathx.GameCLI.Editor
{
    public static class JiraConnectionClient
    {
        public static async Task<string> TestAsync(
            JiraConnectionSettings settings, string secret, CancellationToken cancellation)
        {
            ...
            using (HttpRequestMessage request = new HttpRequestMessage(
                HttpMethod.Get, settings.address + "/rest/api/2/myself"))
            {
                ...
                try
                {
                    using (HttpResponseMessage response = await client.SendAsync(request, cancellation).ConfigureAwait(false))
                    {
                        ...
                        if (user == null || string.IsNullOrEmpty(user.displayName) ||
                            (string.IsNullOrEmpty(user.accountId) && string.IsNullOrEmpty(user.name)))
                        {
