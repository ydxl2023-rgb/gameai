...
namespace Oathx.Rpc.Editor
{
	...
	/// 
	/// </summary>
	public static class XRpcTreeUtility
	{
		/// <summary>
		/// 
		/// </summary>
		...
		public static void TreeToList<T>(T root, IList<T> result) where T : XRpcTreeElement
		{
			if (result == null)
		...
		/// <returns></returns>
		public static T ListToTree<T>(IList<T> list) where T : XRpcTreeElement
		{
			// Validate input
			...
			for (int parentIndex = 0; parentIndex < list.Count; parentIndex++)
			{
				...
				// Fill child array
				List<XRpcTreeElement> childList = null;
				if (childCount != 0)
				{
					// Allocate once
					childList = new List<XRpcTreeElement>(childCount);
					childCount = 0;
		...
		/// <typeparam name="T"></typeparam>
		/// <param name="list"></param>
		public static void ValidateDepthValues<T>(IList<T> list) where T : XRpcTreeElement
		{
			if (list.Count == 0)
			...
			if (list[0].depth != -1)
			{
				throw new ArgumentException("list item at index 0 should have a depth of -1" +
					" (since this should be the hidden root of the tree). Depth is: " + list[0].depth, "list");
			}
			...
			for (int i = 0; i < list.Count - 1; i++)
			{
				...
				{
					throw new ArgumentException(string.Format("Invalid depth info in input list. Depth cannot increase more than 1 per row. " +
						"Index {0} has depth {1} while index {2} has depth {3}", i, depth, i + 1, nextDepth));
				}
			}
		...
		/// <typeparam name="T"></typeparam>
		/// <param name="root"></param>
		public static void UpdateDepthValues<T>(T root) where T : XRpcTreeElement
		{
			if (root == null)
		...
		/// <returns></returns>
		static bool IsChildOf<T>(T child, IList<T> elements) where T : XRpcTreeElement
		{
			while (child != null)
			{
		...
		/// <param name="elements"></param>
		/// <returns></returns>
		public static IList<T> FindCommonAncestorsWithinList<T>(IList<T> elements) where T : XRpcTreeElement
		{
			if (elements.Count == 1)
